...
export default function AnalysisScreen() {
  ...
  const [analyzingStatus, setAnalyzingStatus] = useState("Initializing model weights...");
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      try {
        setHasError(null);
        // Wait at least 2.5s to ensure full radar scan animation delight
  ...
  const handleStepChange = (step: number) => {
    if (step === 1) setAnalyzingStatus("Applying CLAHE contrast enhancement...");
    if (step === 2) setAnalyzingStatus("Extracting microvascular features & foveal zone...");
    if (step === 3) setAnalyzingStatus("Ensemble neural model compiling diagnosis...");
  };
    ...
    <View testID="analysis-screen" style={[styles.root, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      ...
      <View style={styles.header}>
        <View style={styles.modeBadge}>
          <Text style={styles.modeBadgeText}>
            {demoMode ? "DEMO MODE ACTIVE" : "REAL AI VISION ACTIVE"}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Our AI is screening the image for retinal features associated with diabetic retinopathy.
        </Text>
        ...
        {hasError ? (
          ...
          <View style={styles.scannerWrapper}>
            <NeuralScannerRing
              imageUri={selectedImageUri || selectedSample?.image_url}
